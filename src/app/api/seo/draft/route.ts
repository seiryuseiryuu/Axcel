...
8. 画像・内部リンクの配置【重要・ユーザー要望】
   - **アイキャッチ画像**: 
     ${includeEyeCatch
                ? `記事のメリハリをつけるため、各H2見出しの下に**挿入すべき画像の簡単な説明**を必ず配置してください。
         - 形式: \`[EYECATCH: 画像の簡単な説明（日本語で1-2文）]\`
         - 例: \`[EYECATCH: カフェでノートパソコンを使って作業している若い女性]\`
         - 説明は記事の内容に即した具体的なシーンを日本語で簡潔に記述してください。
         - 抽象的な概念ではなく、視覚的に描写できる具体的なシーンを書いてください。`
