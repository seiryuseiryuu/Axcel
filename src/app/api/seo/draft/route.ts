...
${internalLinksText ? `8. 内部リンク（自社メディア記事の活用）【重要】
   ...
   【まとめセクションでの活用】
   - まとめ部分では、記事の内容を振り返りつつ、挿入した内部リンク（またはリストにある候補）を再度提示し、「合わせて読むことで理解が深まる」という文脈で誘導してください。
   - CTA（Call To Action）も、これらの内部リンク先（または最も重要なコンバージョンポイント）へ自然に繋がるように設計してください。
