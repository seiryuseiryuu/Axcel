...
${h2Summary}`;
        ...
        // 内部リンク情報の整形 (Style Analysis)
        let internalLinksText = "";
        let internalLinksStyleMsg = "";
        ...
        if (internalLinks && internalLinks.length > 0) {
            ...
            // 内部リンク候補がある場合、そのスタイルを参照するように指示
            internalLinksStyleMsg = `
【重要】文体・スタイルの統一指示（内部リンク候補の参考）
今回は「${toneGuide[tone]}」を指定されていますが、さらに**上記の「内部リンク候補」の記事（${internalLinks.map(l => l.title).join(", ")}など）と同じWebサイト内の記事**であることを強く意識してください。
- サイト全体で統一感のある「挨拶」「語り口」「まとめ方」を採用すること。
- もし内部リンク候補のタイトルから「親しみやすさ」「硬さ」などのニュアンスが読み取れる場合は、それに合わせること。
- (例: 「初心者向け」のタイトルが多いなら、優しく噛み砕いた表現にする)
`;
        }
...
【最重要指示：最新情報の扱いについて】
これより下に記載する「最新情報・公式リンク」のセクションには、Web検索によって取得したリアルタイムの事実（${new Date().getFullYear()}年の最新状況、最新モデル、バージョン情報など）が含まれています。
...
【重要：文字数に関する厳格な指示】
指定された目標文字数（${wordCountMin}文字以上）は、**記事の品質を担保するための必須要件**です。
- 文字数が不足しそうな場合は、各セクションで「具体的な事例」「理由の深掘り」「メリット・デメリットの比較」「よくある失敗談」などを追加して、内容を充実させてください。
- 単なる引き伸ばしは禁止ですが、読者の理解を助けるための詳細な解説は積極的に行ってください。
...
8. 画像・内部リンクの配置【重要・ユーザー要望】
   - **アイキャッチ画像**: 
     ${includeEyeCatch
                ? `記事のメリハリをつけるため、各H2見出しの下に**挿入すべき画像の詳細な説明**を必ず配置してください。
         - 形式: \`[EYECATCH: 具体的で詳細な画像説明]\`
         
         【重要】画像説明には以下を必ず含めてください：
         1. **被写体**: 誰が/何が写っているか（年齢、性別、服装、表情を具体的に）
         2. **シーン/背景**: どこで撮影されたか（オフィス、カフェ、自宅など）
         3. **構図**: 正面、横顔、俯瞰、クローズアップなど
         4. **雰囲気/ムード**: 明るい、落ち着いた、プロフェッショナルなど
         5. **スタイル**: 写実的、イラスト風、ビジネス風など
         
         ✅ 良い例:
         - \`[EYECATCH: 30代のビジネスマンがノートPCで作業する姿、明るいカフェの窓際席、正面構図、自信に満ちた表情、ビジネスカジュアルな服装、暖かい自然光]\`
         - \`[EYECATCH: 白いテーブルの上に置かれた3つのスマートフォン比較、俯瞰撮影、クリーンな白背景、商品カタログ風、シャープな照明]\`
         
         ❌ 悪い例:
         - \`[EYECATCH: ビジネスの成功]\` ← 抽象的すぎる
         - \`[EYECATCH: スマホを持つ人]\` ← 詳細不足`
                : `**アイキャッチ画像のプレースホルダーは不要です。** 画像に関する記述は一切含めないでください。`
            }
   ...
   【まとめセクションでの活用・重要】
    ...
    - **まとめの書き方は、上記「内部リンク候補（Webサイト内の既存記事）」の雰囲気や、参考記事の「まとめ」セクションを参考にしてください。**
    - サイト全体で統一感のあるまとめ（例：最後に必ず読者へのエールを入れる、次の行動を促すなど）を意識してください。
